...
def build_brief_pdf(theme, articles, brief_text, brief_type="full", db_path=None):
    ...
    if article_ids and db_path:
        ...
        for r in rows:
            all_charts.append({"id": r[0], "article_id": r[1], "caption": r[2],
                "description": r[3], "insight": r[4], "image_data": r[5],
                "width": r[6], "height": r[7]})
    ...
    story.append(Paragraph(
        f"Meridian Intelligence  .  {today}  .  {len(articles)} articles  .  {len(sources)} sources", SM))
    ...
    for sec in sections:
        ...
        if brief_type == "full" and all_charts and total_used < MAX:
            ...
            selected = score_charts_for_section(h + " " + b, candidates,
                max_charts=min(4, MAX - total_used))
            if selected:
