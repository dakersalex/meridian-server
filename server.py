...
class EconomistScraper:
    ...
    def scrape(self):
        ...
        if sub.returncode != 0:
            ...
            if "ECONNREFUSED" in sub.stderr or "connect" in sub.stderr.lower():
                log.error("Economist CDP launch failed — Chrome did not start on port " + str(self.CDP_PORT))
                with sqlite3.connect(DB_PATH) as _cx:
...
def enrich_via_browser_save(aid):
    """Receive full article text from Chrome MCP enrichment and save it."""
    data = request.json or {}
    body = data.get("body", "").strip()
    if not body or len(body) < 100:
        return jsonify({"ok": False, "error": "Body too short"})
    with sqlite3.connect(DB_PATH) as cx:
        cx.execute(
            "UPDATE articles SET body=?, status='fetched', fetched_at=? WHERE id=?",
            (body, now_ts(), aid)
        )
        changed = cx.execute("SELECT changes()").fetchone()[0]
    if changed:
        # Trigger AI enrichment (summary/topic/tags) in background
        row = None
        with sqlite3.connect(DB_PATH) as cx:
