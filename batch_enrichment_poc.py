...
Based on the title, source, and context, provide the analysis."""
        ...
        # Format for Batch API - correct format per docs
        request = {
...
def submit_batch(requests_data):
    ...
    with open(BASE_DIR / "logs" / "batch_payload.json", "w") as f:
        json.dump(payload, f, indent=2)
    print(f"📝 Saved batch payload to logs/batch_payload.json")
...
def download_results(batch_id):
    ...
    with open(BASE_DIR / "logs" / "batch_results.jsonl", "w") as f:
        f.write(response.text)
    print(f"📝 Saved batch results to logs/batch_results.jsonl")
